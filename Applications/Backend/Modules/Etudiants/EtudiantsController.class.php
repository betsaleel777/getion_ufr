<?php
namespace Applications\Backend\Modules\Etudiants ;

class EtudiantsController extends \Library\BackController
{
    public function executeIndex(\Library\HTTPRequest $request)
    {
        $this->page->addVar('title', 'Etudiants');
        $this->setView('index');
        ...
        $this->page->addVar('titre', 'Liste Etudiants') ;
        $this->page->addVar('tableau', $tableau) ;
    }

    public function executeAdd(\Library\HTTPRequest $request)
    {
        $this->page->addVar('title', 'Ajout Etudiants');
        $this->setView('add');
        if ($request->postExists('uniqid')) {
            $retour = $this->processForm($request, 'Etudiants') ;
            if(!empty($retour)){
    ...
    {
      $this->page->addVar('title', 'Modifier Etudiants');
      $this->setView('update');
      if ($request->postExists('uniqid')) {
          $this->processForm($request, 'Etudiants') ;
      } else {
          $formBuilder = new \Library\Models\FormsBuilder(
          $this->module(),
          $this->managers(),
    ...
    {
      $this->managers->getManagerOf($this->module())->delete($request->getData('id')) ;
