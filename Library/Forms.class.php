...
class Forms {
    ...
    public function __construct($uniqid, $method = 'get', array $message=null) {

        $this->fields= array();
        $this->errors_message = $message ;
        ...
        $this->attrs         = new \Library\AttributeList(array('method' => $method));
    ...
    public function add($field, $name) {
        ...
        if (!isset($this->fields[$name])) {
            ...
            $field = '\Library\Form_'.ucfirst($field);
    ...
    protected function _html_fields($tab, $fields, $filter = array()) {
        ...
        foreach($fields as $f) {
            ...
            if (empty($filter) || in_array($f->get_name(), $filter)) {
                $champ = $f->get_name() ;
                if(array_key_exists($champ,$this->errors_message)) {
                  $message = $this->errors_message[$champ] ;
                }
                $o .= "$tab<div class='form-group'>\n".$f->__toString($tab."\t").$message."\n$tab</div>\n";
            }
        }
        return $o;
    }
...
abstract class Form_Field {
    ...
    public function __construct($name, $form) {
        ...
        $this->value    = '';
        $this->class    = array();
        $this->attrs    = new \Library\AttributeList;
        $this->attrs['name'] = $name;
        $this->error_messages= new \Library\ErrorList;
